...
clase = "DUMMY0"
TERP_path = './'
random_frames = np.array(np.loadtxt('gamma1e-3_frames_level_set_DUMMY1.txt'), dtype = int)
for frame in random_frames:
	...
	np.save(TERP_path + 'DATA/TERP_predictions.npy', z_mean.detach().numpy().ravel())
	subprocess.call('python ' + TERP_path + 'TERP_optimizer_01.py -TERP_input DATA/TERP_numeric.npy -blackbox_prediction ' + TERP_path + 'DATA/TERP_predictions.npy --euclidean', shell = True)
	...
	outputs, z_sample, z_mean, z_logvar = IB.forward(torch.Tensor(predictions))
	predicted_prob = torch.exp(outputs)
	np.save(TERP_path + 'DATA_2/TERP_predictions.npy', z_mean.detach().numpy().ravel())
	subprocess.call('python TERP_optimizer_02.py -TERP_input DATA_2/TERP_numeric.npy -blackbox_prediction DATA_2/TERP_predictions.npy -selected_features TERP_results/selected_features.npy --euclidean', shell = True)
