...
frames = np.loadtxt('random_frames19.txt')
print(frames.shape)
...
clase = "9"
TERP_path = './'
random_frames = np.array(np.loadtxt('random_frames19.txt'), dtype = int)
for frame in random_frames:
    ...
    np.save(TERP_path + 'DATA/TERP_predictions.npy', z_mean.detach().numpy())
    ...
    outputs, z_sample, z_mean, z_logvar = IB.forward(torch.Tensor(predictions))
    predicted_prob = torch.exp(outputs)
    np.save(TERP_path + 'DATA_2/TERP_predictions.npy', z_mean.detach().numpy())
